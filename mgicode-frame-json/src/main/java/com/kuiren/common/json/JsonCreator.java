...
public class JsonCreator {
	 ...
	 * 
	 * @Override public String convert(Object object, String name, Object val) {
	 *           if (object instanceof TCommonType) { String dname = dictService
	...
	/**
	 * propname只有在使用IJSonConvert回调时使用 .add("data", "children", new
	 * IJsonConvert() {
	 * 
	 ...
	 * 
	 *           add第二个参数表达式只能指定，不能采用泛指，上面采用泛指，会出现一直执行的问题， 而add(String name,
	 *           Object value) 则不会
	 * @author:彭仁夔 于2014年10月25日上午7:17:54创建
	 * @param name
	 * @param value
	 * @param value
	 *            需要进行转换的名称（如id采用回调函数转为name）
	 * @return
	 */
	...
	public String build(Object o) {
		try {
			return buildNode("actionbean", "actionbean", o,
					new StringBuilder(), false, new ArrayList<String>());
		} catch (Exception e) {
			e.printStackTrace();
		}
		return "";
	...
	private boolean needBuild(String targetName) {
		return ((onlyFlag == false) || (onlyFlag == true && StringUtil.contain(
				onlyNameList, targetName)));
	}

	public String buildNode(String dottargetName, String targetName, Object in,
			StringBuilder out, boolean incollect, List<String> parents)
			throws Exception {
		...
		if (needBuild(targetName)) {
			...
			if (Collection.class.isAssignableFrom(in.getClass())) {
				...
				buildCollection(dottargetName, targetName, in, out, incollect,
						parents);

			} else if (in.getClass().isArray()) {

				buildArray(dottargetName, targetName, in, out, incollect,
						parents);
				...
				buildObject(dottargetName, targetName, in, out, incollect,
						parents);
			}
		}
	...
	private String getSimpleTypeStr(Object property, int flag) {
		...
		if (String.class.isAssignableFrom(type)) {
			...
			if (this.dataFormat != null) {
				...
				return JsonUtil.quote(DateUtil.getDateFormat((Date) property,
						dataFormat));
	...
	private void buildCollection(String dottargetName, String targetName,
			Object in, StringBuilder out, boolean incollect,
	...
	private void buildArray(String dottargetName, String targetName, Object in,
			StringBuilder out, boolean incollect, List<String> parents)
			throws Exception {
	...
	private Method getReadMethod(PropertyDescriptor property, Class clazz) {
		...
		if (clazz != null && clazz.getName().indexOf("$$") > -1) { // 如果是CGLIB动态生成的类
			try {
				String name = clazz.getName().substring(0,
	...
	// return null;
	// }
	private PropertyDescriptor[] getPropertyDescriptors(Class clazz, Object in)
			throws Exception {
	...
	private void buildMap(String dottargetName, String targetName, Object in,
			StringBuilder out, boolean incollect, List<String> parents)
			throws Exception {
		out.append("{");
		...
		Map<?, ?> pdmap = initMapConfig(dottargetName, targetName, in,
				(Map<?, ?>) in, addlist);
		...
		for (Map.Entry<?, ?> entry : ((Map<?, ?>) pdmap).entrySet()) {
			...
			if (value != null
					&& value.getClass().getName().trim()
							.equals("java.sql.Clob")) {
			...
			if (JsonUtil.isSimpleType(value)) {
				...
				if (needBuild(propertyName)) {
					...
					buildNode(dottargetName, propertyName, value, out, true,
							parents);
				}
			}
		}
	...
	private void buildObject(String dottargetName, String targetName,
			Object in, StringBuilder out, boolean incollect,
			List<String> parents) throws Exception {
		...
		// 初始化增加列
		List<JsonConfig> addlist = new ArrayList<JsonConfig>();
		List<PropertyDescriptor> pdlist = initObjConfig(dottargetName,
				targetName, in, props, addlist);

		for (PropertyDescriptor property : pdlist) {
			String key = property.getName();
			// 默认去掉class的属性，这个是java对象本身的
			if (JsonUtil
					.isExcludedType(property.getPropertyType(), Class.class)) {
				continue;
			}
			try {
				...
				if (readMethod != null) {
					...
					buildProp(dottargetName, key, value, out, oldLength, 0,
							parents, in);
				}
			} catch (Exception e) {
				e.printStackTrace();
	...
	private boolean buildProp(String dottargetName, String key, Object value,
			StringBuilder out, int oldLength, int flag, List<String> parents,
		...
		if (JsonUtil.isSimpleType(value)) {
			...
			if (needBuild(key)) {
				...
				List<String> parentList = ListUtil.copy(parents);
				parentList.add(in.toString());
				buildNode(dottargetName + "." + key, key, value, out, true,
						parentList);
			}
		}
	...
	private void configPropDeal(List<JsonConfig> thisAddList, Object in,
			String dottargetName, StringBuilder out, int oldLength,
			List<String> parents) throws Exception {
		for (JsonConfig kv : thisAddList) {
			...
			if (value instanceof IJsonConvert) {// add支持回调函数
				...
				if ("10".equals(kv.getType())) { // replace
					value = ((IJsonConvert) value).convert(in,
							kv.getActualName(),
			...
			buildProp(dottargetName, addName, value, out, oldLength, flag,
					parents, in);
		}
	}
	...
	private String calAddName(JsonConfig kv, Object in) {
		...
		if (kv.getAddName() instanceof INameConvert) {
			addName = kv.getAddName(in, kv.getActualName(),
					JsonUtil.evalByOgnl(kv.getExpr(), in));
		} else {
	...
	private List<PropertyDescriptor> initObjConfig(String dottargetName,
			String targetName, Object in, PropertyDescriptor[] props,
		...
		for (PropertyDescriptor prop : props) {
			for (JsonConfig jc : configs.get("rep")) {
				if (jsonMatch.matchUpdate(dottargetName, targetName, in, prop,
						jc)) {
			...
			for (JsonConfig jc : configs.get("del")) {
				if (jc.getDelCls() != null
						&& JsonUtil.isExcludedType(prop.getPropertyType(),
								jc.getDelCls())) {
					pdMap.remove(prop.getName());
				} else if (jsonMatch.matchDel(dottargetName, targetName, in,
						prop, jc)) {
		...
		return ListUtil.collect2List(pdMap.values());
	}

	private Map initMapConfig(String dottargetName, String targetName,
			Object in, Map<?, ?> props, List<JsonConfig> addlist) {
		...
		for (Object prop : props.keySet()) {
			...
			for (JsonConfig jc : configs.get("rep")) {
				if (jsonMatch.matchUpdate(dottargetName, targetName, in,
						(String) prop, jc)) {
					Object value = jc.getValue();
					if (value == null) {
						value = JsonUtil.evalByOgnl(jc.getExpr(), in);
			...
			for (JsonConfig jc : configs.get("del")) {
				if (jc.getDelCls() != null
						&& JsonUtil.isExcludedType(vObj.getClass(),
								jc.getDelCls())) {
