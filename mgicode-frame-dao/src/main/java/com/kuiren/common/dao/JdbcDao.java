package com.kuiren.common.dao;

import java.util.List;

import org.springframework.jdbc.core.JdbcTemplate;

/**
 * @author pengrk
 * @email:sjkjs155@126.com
 * @wetsite:www.mgicode.com
 * @license:GPL
 */
public interface JdbcDao <T, PK > {
	
    /**
     * 保存对象
     * @param entity
     */
	public abstract void add(T entity);

	/**
	 * 更新对象
	 * 
	 * @param object
	 */
	public abstract void update(T object);

	/**
	 * 按照指定条件和参数进行查询操作
	 * 
	 * @param conditionSql
	 *            条件sql语句 如 name=? && password=?
	 * @param params
	 *            参数 如 new Object[]{"f","f"}
	 * @return 存放对象的列表
	 */
	public abstract List<T> query(String conditionSql, Object[] params);

	/**
	 * 使用sql语句进行查询
	 * 
	 * @param sql
	 *            sql语句
	 * @param params
	 *            参数 如 new Object[]{"f","f"}
	 * @return 存放对象的列表
	 */
	public abstract List<T> queryBySql(String sql, Object[] params);
	

	/**
	 * 使用sql语句进行查询,返回3,5,4这样形式的ids
	 * 
	 * @param sql
	 *            sql语句
	 * @param params
	 *            参数 如 new Object[]{"f","f"}
	 * @return 存放对象的列表
	 */
	public String  queryIdsBySql(String sql, Object[] params);

	/**
	 * 按照指定条件和参数进行删除操作
	 * 
	 * @param conditionSql
	 *            条件sql语句 如 name=? && password=?
	 * @param params
	 *            参数 如 new Object[]{"f","f"}
	 * @return 存放对象的列表
	 */
	public abstract void delete(String conditionSql, Object[] params);

	/**
	 * 查询指定ID的对象
	 * 
	 * @param id
	 * @return
	 */
	public abstract T queryById(String id);

	/**
	 * 根据属性名和属性值查询对象.
	 * 
	 * @return 符合条件的对象列表
	 */
	public abstract List<T> queryBy(String propertyName, Object value);

	/**
	 * 得到所有的对象
	 */
	public abstract List<T> queryAll();

	/**
	 * 删除指定ID的对象
	 * 
	 * @param id
	 */
	public abstract void deleteById(String id);

	/**
	 * 删除所有的对象
	 */
	public abstract void deleteAll();

	/**
	 * 得到主键的名称
	 */
	public abstract String primaryKeyName();

	/**
	 * 得到表格的名称
	 */
	public abstract String tableName();
	
	

   /**
    * 取得基于Spring的 JDBC Template
    * @return
    */
   public JdbcTemplate getJdbcTemplate();
   
}
   
 
	